    ...
    for (let i = 0; i < workflows.length; i++) {
        ...
        if (workflow.state == 'active') {
            ...
            for (let i = 0; i < branches.length; i++) {
                ...
                print_menu(output, branchSeparator,branch.conclusion, branch.head_branch, { html_url: branch.html_url })           
                print_menu(output, commitMessageSeperator,branch.conclusion, branch.head_commit.message, { html_url: branch.html_url })

                for (let j = 0; j < jobs.length; j++) {
                    const job = jobs[j]
                    for (let k = 0; k < job.steps.length; k++) {
                        const step = job.steps[k]
                        print_menu(output, subMenuSeperator, step.conclusion, step.name, { itemDuration: step, html_url: job.html_url })
                    }
                }
                status.push({
