...
module.exports.getWorkflows = async () => {
  try {
    ...
    quit('You must configure .xbar-cicd-supervisionrc');
  }
}

module.exports.getRuns = async (workflow) => {
  try {
...
module.exports.getLastRunFromBranch = async (branch) => {
  try {
    ...
    if (res && res.data && res.data.workflow_runs) {
      ...
      if (!triggeredRun) console.log('No run for this branch')
      return triggeredRun
    }
...
module.exports.groupRunsByBranch = async (workflow) => {
  const runs = await this.getRuns(workflow.id)
  const branches = []
